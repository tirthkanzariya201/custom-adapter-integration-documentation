...
Use the exact snippet shown on your platform's integration page.
...
| Wrong class on wrapper vs native | Wrappers use the AdMob class for GAM; native uses the dedicated GAM class | Native: `…TapMindMediationAdapterGAM`. Flutter/RN/Unity/Cocos: use the AdMob class per the Registry note |
...
| Can't tell whether the adapter loaded | — | Run the **MAX Mediation Debugger** in a debug build to confirm the TapMind adapter loads and view its status |
...
Consumer rules cover both the standard adapter namespace (`com.tapmind.tech.*`) and the Next-Gen namespace (`com.tapmind.mediation.ng.*`).
...
- **Android:** filter logcat by `TapMindAdapter` (the Orchestration SDK uses `ORCH`).
- **iOS:** filter the Xcode console / device log by the TapMind subsystem. _[Insert the exact iOS log subsystem or filter string used by the SDK.]_
...
- Log output filtered by `TapMindAdapter` (Android) or the iOS subsystem
