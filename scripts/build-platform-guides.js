/**
 * NOTE: GitBook source tree tapmind-custom-adapter-sdk-integration/ was removed
 * so JamDesk Ask AI no longer indexes legacy content. Published guides under
 * guides/ are the source of truth. Edit those MDX files directly. This script
 * will fail until contentRoot is re-pointed or retired.
 */
const fs = require('fs');
...
const PLATFORMS = [
  {
    ...
    description:
      'Native Android integration guide for TapMind Custom Adapter across AdMob, Google Ad Manager, AppLovin, and IronSource LevelPlay.',
  ...
  },
  {
    ...
    description:
      'Native iOS integration guide for TapMind Custom Adapter across AdMob, Google Ad Manager, AppLovin, and IronSource LevelPlay.',
    networks: ['admob', 'google-ad-manager', 'applovin', 'ironsource-levelplay'],
  },
  {
    ...
    description:
      'Unity Android integration guide for TapMind Custom Adapter across AdMob, Google Ad Manager, AppLovin, and IronSource LevelPlay.',
    networks: ['admob', 'google-ad-manager', 'applovin', 'ironsource-levelplay'],
  },
  {
    ...
    description:
      'Unity iOS integration guide for TapMind Custom Adapter across AdMob, Google Ad Manager, AppLovin, and IronSource LevelPlay.',
    networks: ['admob', 'google-ad-manager', 'applovin', 'ironsource-levelplay'],
  },
  {
    ...
    description:
      'Flutter Android integration guide for TapMind Custom Adapter across AdMob, Google Ad Manager, AppLovin, and IronSource LevelPlay.',
    networks: ['admob', 'google-ad-manager', 'applovin', 'ironsource-levelplay'],
  },
  {
  ...
  },
  {
    ...
    description:
      'Cocos Android integration guide for TapMind Custom Adapter across AdMob and Google Ad Manager.',
...
function buildPlatform(platform) {
  ...
  const platformIntro = readSection(baseDir, 'README.mdx', 0);
  platformIntro.imports.forEach((i) => allImports.add(i));
  mergedSources.push(`${platform.key}/README.mdx`);
...
`;
  ...
  return {
    key: platform.key,
    slug: `tapmind-custom-adapter-sdk-integration/${platform.key}/integration-guide`,
    outFile: path.relative(rootDir, outFile).replace(/\\/g, '/'),
    mergedSources,
    networks: platform.networks.map((k) => NETWORK_DEFS[k].title),
    imports: [...allImports],
...
fs.writeFileSync(path.join(__dirname, 'platform-build-manifest.json'), JSON.stringify(results, null, 2));

console.log('Built', results.length, 'platform guides:');
for (const r of results) {
  console.log(`  ${r.outFile} (${r.sizeKb} KB, ${r.networks.length} networks)`);
}

module.exports = { PLATFORMS, buildPlatform, results };
